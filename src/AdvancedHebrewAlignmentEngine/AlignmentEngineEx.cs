...
{
    public class AlignmentEngineEx
    {
        ...
        public AlignmentEngineEx(string apiKey)
        {
            _apiKey = apiKey;
            ...
            string englishVerse)
        {
            ...
            for (int i = 0; i < phrases.Count; i++)
            {
                ...
                for (int j = 0; j < targets.Count; j++)
                {
                    ...
                    if (/*tokens[i].Gloss.ToLower().Contains(targets[j].ToLower()) || */targets[j].ToLower() == phrases[i].Gloss.ToLower())
                        score += 0.35;

                    if (phrases[i].Text != "אֵ֥ת")
                    {
...
";
        ...
        // ================= CONTEXT BUILDERS =================
        private string BuildHebrewContext(Phrase p, string reference, string verse)
        {
            return $"In {reference}, Hebrew phrase '{p.Text}' (lemma: {p.Lemma}, pos: {p.POS}, morphology: {p.Morph}) in the verse '{verse}'";
        }
        ...
        private List<ExpandedToken> Expand(List<HebrewToken> tokens)
        {
            ...
            foreach (var t in tokens)
            {
                ...
                list.Add(new ExpandedToken
                {
                    ...
                    POS = t.POS,
                    Morph = t.Morphology,
                    Context = $"Hebrew {t.Lemma} {t.POS} {t.Morphology}",
                    Gloss = t.Gloss
                });
            }

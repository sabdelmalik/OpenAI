...
namespace AdvancedAligner
{
    ...
    {
        public SortedDictionary<int, VersionVerse> VersionBible { get; } = new SortedDictionary<int, VersionVerse>();
        ...
        public List<string> VersionBooks { get; } = new List<string>();
        public VersionParser(string versionFilePath, List<string> tahotBooks)
        {
            ParseVersionFile(versionFilePath, tahotBooks);
        }
...
{"Psa.145.0", "A psalm of praise. Of David."},
        ...
        private void ParseVersionFile(string versionFilePath, List<string> tahotBooks)
        {
            ...
            SortedDictionary<int, VersionVerse> tempBible = new SortedDictionary<int, VersionVerse>();
            ...
            foreach (string line in lines)
            {
                ...
                VersionVerse verse = new VersionVerse(reference, string.Empty, translationText);

                tempIndices[reference] = referenceIndex;
                tempBible[referenceIndex] = verse;
                if (!VersionBooks.Contains(bookName))
                {
                    VersionBooks.Add(bookName);
                }
            ...
            foreach (var kvp in tempBookCounts)
            {
                ...
                int versionBookIndex = VersionBooks.IndexOf(versionBookName);
                if (versionBookIndex < 0 || versionBookIndex >= VersionBooks.Count)
                {
            ...
            // copy tempBible to VersionBible after translating reference to dot notation
            int updatedIndex = 0;
            for (referenceIndex = 0; referenceIndex < tempBible.Count; referenceIndex++)
            {
                VersionVerse verse = tempBible[referenceIndex];
                ...
                if (splitPsalmsTitles && bookName == "Psa" && verseNumber == "1")
                {
                    ...
                    if (psalmsTitles.ContainsKey(psalmKey))
                    {
                        ...
                        VersionBible[updatedIndex] = new VersionVerse(titleReference, titleReferenceDN, psalmsTitles[psalmKey]);
                ...
                VersionBible[updatedIndex] = new VersionVerse(versionReference, dotNotationReference, verseText);
        ...
        {
            int count = 0;
            foreach (var kvp in VersionBible)
            {
